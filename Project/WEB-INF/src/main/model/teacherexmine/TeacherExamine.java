package main.model.teacherexmine;
...
public class TeacherExamine {
	...
	public List<ExamineHistoryTeacher> examine_all() {
		List<ExamineHistoryTeacher> h_result = new ArrayList<>();
		List<ExamineHistoryTeacher> e_result = new ArrayList<>();
		...
		try{
			...
			List<main.datamodel.mapper.History> hl = db.from(history)
											      .select();
			for(main.datamodel.mapper.History h : hl){
				ExamineHistoryTeacher eht = new ExamineHistoryTeacher();
				eht.examine_id = String.valueOf(h.examine_id);
				eht.day = String.valueOf(h.date.toString());
				h_result.add(eht);
			}
			...
			for(ExamineHistoryTeacher r : h_result) {
				...
				main.datamodel.mapper.Examine examine = new main.datamodel.mapper.Examine();
				List<main.datamodel.mapper.Examine> el = db.from(examine)
														 .where(examine.examine_id).is(Integer.parseInt(r.examine_id))
														 .select();
				eht.examine_id = r.examine_id;
				eht.day = r.day;
				eht.examine_name = el.get(0).examine_name;
				eht.promoter_id = String.valueOf(el.get(0).promoter_id);
				e_result.add(eht);
			}
			...
			for(ExamineHistoryTeacher r : e_result){
				ExamineHistoryTeacher eht = new ExamineHistoryTeacher();
				main.datamodel.mapper.Promoter promoter = new main.datamodel.mapper.Promoter();
				List<main.datamodel.mapper.Promoter> pl = db.from(promoter)
														  .where(promoter.promoter_id).is(Integer.parseInt(r.promoter_id))
														  .select();
				eht.examine_id = r.examine_id;
				eht.day = r.day;
				eht.examine_name = r.examine_name;
				eht.promoter_id = r.promoter_id;
				eht.promoter_name = pl.get(0).promoter_name;
				result.add(eht);
			}
		} catch (Exception e){
			db.close();
