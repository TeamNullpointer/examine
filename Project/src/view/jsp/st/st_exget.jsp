...
<table class="line_table contents_center">
 ...
 <tr>
    ...
    %>
    <td><%=isPass %></td>
    <td><form method="post" action="<%=request.getContextPath()%>/st_exdel">
    	<input type="hidden" name="history" value=<%=stExamHis.history_id%>>
    	<input type="hidden" name="user" value=<%=user.mail%>>
    	<button class="color_btn" type="submit" onclick='return confirm("削除します。よろしいですか？");'>削除</button>
    	</form></td>
  </tr>
  <%} %>
</table>
